...
import (
	...
	"gitlab.com/megatech/serverex/apps/identity/controller"
...
func main() {
	...
	flag.StringVar(&cfg, "c", "config.toml", "use a specific config file")
	flag.Parse()
	...
	api := engine.Group("/identity/api")

	svc := service.New(config)
	svc.SetGinRouter(api)

	// 服务启停
	srv := &http.Server{
		Addr:    config.ServerOption.ListenAddress,
		Handler: engine,
